...
server.listen(PORT, async () => {
  ...
  // ── Startup validation ──
  const env = (key) => process.env[key] ? '  ok' : '  MISSING';
  console.log(`  Gemini API Key: ${env('GEMINI_API_KEY')}`);
  console.log(`  GitHub Token:   ${env('GITHUB_TOKEN')}`);
  console.log(`  Service Account:${env('GOOGLE_SERVICE_ACCOUNT_B64')}`);
  console.log(`  Supabase:       ${env('SUPABASE_URL')}`);
  console.log(`  Deepgram:       ${env('DEEPGRAM_API_KEY')}`);
  console.log(`  Resend:         ${env('RESEND_API_KEY')}`);

  // Validate knowledge base at startup
  if (process.env.SUPABASE_URL && process.env.SUPABASE_KEY) {
    try {
      const { createClient } = require('@supabase/supabase-js');
      const sb = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_KEY);
      const { count } = await sb.from('frameworks_knowledge').select('id', { count: 'exact', head: true });
      console.log(`  Knowledge base: ${count} chunks`);
      if (count === 0) console.warn('  WARNING: Knowledge base is empty — tools will return no results');
    } catch (err) {
      console.warn('  WARNING: Knowledge base check failed:', err.message);
    }
  }
  ...
  try {
    const { FrameworkFetcher } = require('./framework-fetcher');
    const tools = FrameworkFetcher.getGeminiFunctionDeclarations();
    console.log(`  Tools:          ${tools.length} declared (${tools.map(t => t.name).join(', ')})`);
  } catch (err) {
    console.warn('  WARNING: Tool declarations failed:', err.message);
  }
